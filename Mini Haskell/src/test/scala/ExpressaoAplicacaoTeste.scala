...
import Tipos.{Aplicacao, DecFuncao, ExpressaoLet, ExpressaoSoma, Referencia, ValorInteiro}
...
class ExpressaoAplicacaoTeste extends FlatSpec with Matchers {
  ...
  "uma aplicacao inc 4 (onde existe inc x = x + 1) " should " levar ao valor 5" in {
    ...
    val inc = new DecFuncao("inc", List("x"), new ExpressaoSoma(new Referencia("x"), ValorInteiro(1)))
